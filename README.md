...
The Streamlit app can save visitor questions and DuRu's answers to Supabase. If Supabase secrets are not configured, the app still works and simply skips logging.
...
Then add these secrets in Streamlit Community Cloud:
...
Do not put Supabase keys in GitHub. Store them only in Streamlit secrets. After redeploying, open Supabase `Table Editor` -> `chat_logs` to see saved conversations.
